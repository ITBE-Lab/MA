...
template<>
__device__ 
float gpuMax( float first, float second )
{
...
template<
	...
	__global__
	void tileKernel16( uint2* pvHF_VectorIn, // Vector has size of reference sequence H1, H2, F1, F2
					   ...
					   uint2* pvHE_CacheIn, // H1, H2, E1, E2 size: (gridDim.x * blockDim.x) * STRIPE_WIDTH equal to uiNumberOfSegments * STRIPE_WIDTH
...
{	/* !! NUM_OF_SEGMENTS must be equal to gridDim.x * blockDim.x !! */
	...
	const unsigned int iv2GapOpenScore = ((GAP_INIT + GAP_EXT) << 16) | (GAP_INIT + GAP_EXT); // init-gap 16-bit, init-gap 16 bit
	const unsigned int iv2GapExtScore = (GAP_EXT << 16) | GAP_EXT; // gap-ext 16-bit, gap-ext 16 bit
	...
	*/
...
#if ( USE_REGISTER_CACHE == 1 )
				HE_Cache[uiColIndex] = pvHE_CacheIn[uiGlobalMemIndex]; // get H1, H2, E1, E2 from previous block
#else
	...
	{	/* The very first segment is special here and has to be initialized always by 0 and H from the final row*/
	...
	{	
		...
		const uint2 i2_HF_Cell = pvHF_VectorIn[uiIndex]; // H1(short), H2(short), E1(short), E2(short)
		...
		unsigned int iH_ValueMax = pvM_VectorIn[uiIndex]; // M1(short), M2(short) previously i2_MC_CellIn.x;
		...
		for( unsigned short uiColIndex = 0; uiColIndex < STRIPE_WIDTH; ++uiColIndex )
		{
			 ...
			 * Implements: iE_Value = gpuMax( iH_ValueUp - (GAP_INIT + GAP_EXT), iE_ValueUp - GAP_EXT );
			 ...
			 */
			iE_Value = __vmaxs2( __vsub2( iH_ValueUp, iv2GapOpenScore ),  __vsub2( iE_ValueUp, iv2GapExtScore ) );	 
			 ...
			 * We do two lookups in the scoring profile (one for left symbol, one for right symbol) and 
			 * combine the two scores in one 32 bit integer.
			 */
			const unsigned int iv2_Score = ((reinterpret_cast<const unsigned int *>(&aProfileCache[uiColIndex]))[iRightSymbol] ) & 0x0000FFFF |
										   ((reinterpret_cast<const unsigned int *>(&aProfileCache[uiColIndex]))[iLeftSymbol] << 16);
#else
			 ...
			 * Implements: iF_Value = gpuMax( iH_ValueLeft - (GAP_INIT + GAP_EXT), iF_ValueLeft - GAP_EXT );
			 */
			iF_Value = __vmaxs2( __vsub2( iH_ValueLeft, iv2GapOpenScore ),  __vsub2( iF_ValueLeft, iv2GapExtScore ) );	
...
#if ( USE_CHECKSUM == 1 )
			/* Checksum Update (Remember Checksums must stay 32 bit values, in order to avoid overflows)
			 * Implementation: iHE_Checksum += iH_Value + iE_Value;
			 * Idea: One 32-bit checksum for left side and right side simultaneously.
			 * Idea: Implement the checksum by using one large 64-bit long value
			 */
...
template<typename SCORE_TP4, typename SCORE_TP2, typename SCORE_TP, typename CHECKSUM_TP,
		 unsigned int TILE_PER_BLOCK, 
		 unsigned int STRIPE_WIDTH, 
		 bool LAZY_MODE,
		 bool FULL_STRIPE_WIDTH> // ignore parameter uiWidth, use full stripe width instead.
__global__
void tileKernel( SCORE_TP2* pvHF_VectorIn, // Vector has size of reference sequence
				 ...
				 SCORE_TP2* pvHE_CacheIn, // size: (gridDim.x * blockDim.x) * STRIPE_WIDTH equal to uiNumberOfSegments * STRIPE_WIDTH
...
#endif
				 char* pContinuationFlagVector, // Only used in the lazy-mode to communicate the need of a further kernel call.
				 const unsigned int uiSegmentSize ,// uiNumberOfSegments * uiSegmentSize is equal to the total size 
				 const unsigned int uiWidth // number of columns computed with current stripe ( assert(uiWidth <= STRIPE_WIDTH) )
			   )
{	/* !! NUM_OF_SEGMENTS must be equal to gridDim.x * blockDim.x !! */
	const unsigned int &tid = threadIdx.x;
	const unsigned int &bid = blockIdx.x;
	...
	unsigned int uiSegmentId = (bid * blockDim.x + tid);
	unsigned int uiNumberOfSegments = gridDim.x * blockDim.x;
	unsigned int uiTotalSize = gridDim.x * blockDim.x * uiSegmentSize; // uiNumberOfSegments * uiSegmentSize
	...
	for( unsigned int uiColIndex = 0; uiColIndex < (FULL_STRIPE_WIDTH ? STRIPE_WIDTH : uiWidth); ++uiColIndex )
	{	/* The threads write parallel to consecutive addresses.
		 * This avoids banking conflicts.
		 */
		//// if( !LAZY_MODE || ( uiSegmentId == 0 ) )
	 ...
	 * Threads read consecutive addresses in parallel (but misaligned)
	 */
	SCORE_TP iH_ValueLeftUp = uiSegmentId > 0 ? pvHF_VectorIn[uiTotalSize - uiNumberOfSegments + (uiSegmentId - 1)].x
											  : xLeftUpH_Value; //  0; /* The very first segment is special here and has to be initialized always by 0 */
	...
	{	
		...
		for( unsigned int uiColIndex = 0; 
			 uiColIndex < (FULL_STRIPE_WIDTH ? STRIPE_WIDTH : uiWidth); 
			 ++uiColIndex )
		{
#if ( USE_REGISTER_CACHE == 1 )
			...
			/* The below statement results in the catch of a single int from global memory. */
#if	( SCORING_PROFILE_IN_SHARED_MEMORY == 1 )
			SCORE_TP iScore = (reinterpret_cast<const SCORE_TP *>(&aProfileCache[uiColIndex]))[iSymbol];
#else
			SCORE_TP iScore = (reinterpret_cast<const SCORE_TP *>(&i4_Scores[uiColIndex]))[iSymbol];
#endif
			/* Update the F-value for the current cell.
			 * At this moment iH_Value still contains the H-value of the left cell.
			 */
			iF_Value = gpuMax( iF_Value - GAP_EXT_HORIZONAL, iH_Value - (GAP_INIT + GAP_EXT_HORIZONAL) );
			//// before iF_Value = gpuMax( iF_ValueLeft - GAP_EXT_HORIZONAL, iH_ValueLeft - (GAP_INIT + GAP_EXT_HORIZONAL) );
			
			/* iH_ValueLeft keeps currently up-left H-value.
			 * By this assignment we destroy the knowledge about the left H-value. (This is not needed any more)
			 */
			iH_Value = gpuMax( iH_ValueLeftUp + iScore, iE_Value );
		...
		//// Jan. 27 /* Prepare the int 4 value for logging the values of the last column into the pHFM_Vector.
		//// Jan. 27  */
		//// Jan. 27 i2_HF_CellOut.x = iH_Value; // i4_Cell.x = iH_Value; // H-value of last column
		//// Jan. 27 i2_HF_CellOut.y = iF_Value; // i4_Cell.y = iF_Value; // F-value of last column
		//// Jan. 27 
		//// Jan. 27 /* Update the cell in the vector with the values of the current column */
		//// Jan. 27 pvHF_VectorOut[uiIndex] = i2_HF_CellOut;
		...
		{  /* The threads write parallel to consecutive addresses.
			...
			*/
			...
			char bE_differend =	   (HE_Cache[uiColIndex].x != pvHE_CacheIn[uiGlobalMemIndex].x)
								|| (HE_Cache[uiColIndex].y != pvHE_CacheIn[uiGlobalMemIndex].y);
#else
			unsigned int uiCacheIndex = blockDim.x * uiColIndex + tid; // blockDim.x is equal to TILE_PER_BLOCK
			char bE_differend =	   (HE_Cache[uiCacheIndex].x != pvHE_CacheIn[uiGlobalMemIndex].x)
								|| (HE_Cache[uiCacheIndex].y != pvHE_CacheIn[uiGlobalMemIndex].y);
#endif		
			if( bE_differend )
