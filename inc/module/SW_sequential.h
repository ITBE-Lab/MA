...
template <class T> class alignment_description_element
{
  ...
  public:
    ...
    alignment_description_element( e_alignment_element_kind_t eElementKind, T entryOnRowSide,
                                   T entryOnColumnSide )
...
class alignment_description : public std::vector<alignment_description_element<T>>
{
  private:
    ...
    void appendSingleRow( typename std::vector<alignment_description_element<T>>::iterator begin,
                          ...
                          std::string &text, eSelectionType selectedSide )
    {
        std::for_each( begin, end,
                       [&]( alignment_description_element<T> &element ) {
  ...
  public:
    ...
    vectorIterator vIterateForwardWhile(
        vectorIterator forwardIterator,
    ...
    std::shared_ptr<std::stringstream> createTextForPositionInformation(
        vectorIterator &rSectionStartIterator, vectorIterator &rSectionEndIterator,
        const size_t uxStartForAbsolutePosition, const size_t uxReverseStrandSize )
    {
        ...
        if( uxReverseStrandSize == 0 )
        {
            ( *pStringStreamRef ) << ( uxStartForAbsolutePosition + rSectionStartIterator ) -
                                         this->begin( ) + 1
                                  << " - "
                                  << ( uxStartForAbsolutePosition + rSectionEndIterator ) -
                                         this->begin( );
        } // if
        else
        {
            ( *pStringStreamRef )
                << uxReverseStrandSize -
                       ( uxStartForAbsolutePosition + rSectionEndIterator - this->begin( ) ) + 1
                << " - "
                << uxReverseStrandSize -
                       ( uxStartForAbsolutePosition + rSectionStartIterator - this->begin( ) );
        } // else
...
template <class SCORE_TP, class T_size_t> struct AlignmentOutcomeMatrix
{
    ...
    AlignmentOutcomeMatrix( T_size_t numberOfColumns, const uint8_t *columnSequence,
        ...
        : puxRowSequenceRef( rowSequence ),
          ...
          numberOfColumns( numberOfColumns +
                           1 ) //+ 1 because the matrix gets a virtual first column
    {
        const T_size_t sizesOfOutcomeAndBacktrackingMatrix =
     ...
     * The caller has to deliver enough space in resultingSequenceForRow as well as
     * resultingSequenceForColumn.
     */
    void backtrackFromIndexText( size_t startIndex, std::vector<char> &resultingSequenceForRow,
                                 std::vector<char> &resultingSequenceForColumn,
                                 size_t &ruiNumberMatches, size_t &ruiNumberMismatches,
                                 size_t &ruiNumberInsertions, size_t &ruiNumberDeletions,
    ...
    void backtrackFromIndex( T_size_t startIndex,
                             ...
                             T_size_t &startPositionInColumn, T_size_t &endPositionInColumn,
                             T_size_t &startPositionInRow, T_size_t &endPositionInRow )
    {
            ...
            switch( direction )
            {
                case LEFT_UP:
                    ...
                    if( puxRowSequenceRef[ uxRow - 1 ] == puxColumnSequenceRef[ uxColumn - 1 ] )
                    {
                         ...
                         */
                        alignmentOutcomeVector.push_back( alignment_description_element<char>(
                            EQUAL_PAIR,
                            NucSeq::translateACGTCodeToCharacter( puxRowSequenceRef[ uxRow - 1 ] ),
                            NucSeq::translateACGTCodeToCharacter(
                                puxColumnSequenceRef[ uxColumn - 1 ] ) ) );
                    } // if
                    else
                    {
                        alignmentOutcomeVector.push_back( alignment_description_element<char>(
                ...
                case LEFT:
                    alignmentOutcomeVector.push_back( alignment_description_element<char>(
                        INSERTION_AT_ROW_SIDE, '+',
                        NucSeq::translateACGTCodeToCharacter(
                            puxColumnSequenceRef[ uxColumn - 1 ] ) ) );
                    break;
                case UP:
                    alignmentOutcomeVector.push_back( alignment_description_element<char>(
                        INSERTION_AT_COLUMN_SIDE,
                        NucSeq::translateACGTCodeToCharacter( puxRowSequenceRef[ uxRow - 1 ] ),
                        '+' ) );
                case STOP:;
            } // switch
    ...
    long long dumpAlignmentFromRowColumn( SCORE_TP score, T_size_t row, T_size_t column,
    ...
    long long dumpAlignmentFromIndex( T_size_t index, SCORE_TP score,
                                      SmithWatermanParamaterSet<SCORE_TP> &rxSW_Parameter )
    {
        ...
        backtrackFromIndexText( index, textBufferforRow, textBufferforColumn, uiNumberMatches,
                                uiNumberMismatches, uiNumberInsertions, uiNumberDeletions,
                                uiNumberGapOpen );
 ...
 * CONF_FILL_OUTCOME_MATRIX if you deliver true here, then the code for filling the outcome matrix
 * is included
 */
template <bool CONF_FILL_OUTCOME_MATRIX, // Compile switch, that decides about filling outcome
                                         // matrix
...
struct SW_align_type
{
     ...
     */
    SW_align_type( size_t numberOfColumns, const uint8_t *columnSequence, size_t numberOfRows,
                   const uint8_t *rowSequence, SmithWatermanParamaterSet<SCORE_TP> &SWparameterSet )
        : numberOfColumns( numberOfColumns ),
     ...
     */
    SCORE_TP swAlign( std::vector<std::pair<size_t, size_t>>
                          &rvMaxScorePositions // vector that receives the max-positions
    )
    {
         ...
         * 2. h_and_e_Columns : Two one dimensional array for intermediate data storage (see
         * ppt-file)
         */
        ...
        for( cNuc = 0; cNuc < alphabetSize; ++cNuc )
        {
            ...
            for( uxIteratorColumn = 0; uxIteratorColumn < numberOfColumns; ++uxIteratorColumn )
            {
                ...
                queryProfile[ queryProfileIterator ] =
                    referenceToStartOfRow[ columnSequence[ uxIteratorColumn ] ];
        ...
        for( uxIteratorRow = 0; uxIteratorRow < numberOfRows; ++uxIteratorRow )
        {
            ...
            /* We select the row in our query profile according to the row-sequence symbol that
             * belongs to the current row
             */
            SCORE_TP *referenceToQueryProfileRow =
                &queryProfile[ rowSequence[ uxIteratorRow ] * numberOfColumns ];

            /* The outcome Matrix has size (numberOfRows + 1) x (numberOfColumns + 1)
             */
            auto uxMatrixStartShift =
                ( ( ( uxIteratorRow + 1 ) * ( numberOfColumns + 1 ) ) + ( 1 ) );

            SCORE_TP *queryOutcomeMatrixIterator =
                alignmentOutcomeMatrix.scoringOutcomeMatrix + uxMatrixStartShift;
            ...
            for( uxIteratorColumn = 0; uxIteratorColumn < numberOfColumns; ++uxIteratorColumn )
            {
                ...
                indexOfMaxScoreInCurrentRow =
                    maxScoreInCurrentRow > h ? indexOfMaxScoreInCurrentRow : uxIteratorColumn;
            ...
            if( maxScoreInCurrentRow >= maxScoreValue )
            {
                ...
                rvMaxScorePositions.push_back(
                    std::pair<size_t, size_t>( uxIteratorRow, indexOfMaxScoreInCurrentRow ) );
            } // if
        } // outer for
