...
struct SW_SIMD_Aligner
{
     ...
     */
    SW_SIMD_Aligner(
        const NucSeq &rQuerySequence, // query sequence of alignment
          ...
          pSWparameterSetRef( SWparameterSet )
    {
         ...
         */
        pH_VectorPreviousRow =
            pAlignedAllocatedMemoryRef + ( uiNumberOfBlocks * SWparameterSet.uiAlphabetSize );
            ...
            for ( T_size_t i = 0; i < uiNumberOfBlocks; ++i )
            {
                ...
                { /* We take std::numeric_limits<SCORE_TP>::min() for the "non existing columns".
                   ...
                   */
                    *profileReference++ =
                        ( k >= uiQueryLen
                              ? std::numeric_limits<SCORE_TP>::min( ) // non existing column
    ...
    )
    {
        ...
        for ( T_size_t uiRow = 0; uiRow < uiLenReference; ++uiRow )
        {
             ...
             */
            T__mXXXi *profileReferenceForRow =
                pAlignedAllocatedMemoryRef + ( pReference[ uiRow ] * uiNumberOfBlocks );
            ...
            { /* SW cells are computed in the following order:
               ...
               */
                /* Compute H(i,j) = max{H(i-1,j-1)+S(i,j), E(i,j), F(i,j)} (parallel for 16/8/4
                 * elements) Note that at the beginning, h = H(i - 1,j - 1)
                 */
                ...
                /* We compute the <f>-values (left cells). These values can be wrong, we correct
                 * this in the lazy-F loop. F(i, j+1) = max{H(i, j) - g_init, F(i, j) - g_ext}
                 */
                f = _mmXXX_subs_epu16( f, gapePar8F ); //// AK
                f = _mmXXX_max_epi16( f, h );

                /* We load h for next iteration. Important is that we do this here and not at the
                 * beginning of the loop.
                 */
                h = _mm_load_siXXX( pH_VectorPreviousRow + j );
            } // for j

            /* The lazy F-loop as described in the paper. This loop is responsible for fixing wrong
            ...
            for ( unsigned int k = 0; k < __mXXXi_SIZE / 2; ++k )
            {
                ...
                for ( T_size_t j = 0; j < uiNumberOfBlocks; ++j )
                {
                    ...
                    /* _mm_movemask_epi8 : Create mask from the most significant bit of each 8-bit
                     * element in a, and store the result in dst. Hint: movemask is simply a trick
                     * in order to finally get an integer that can be used for comparison.
