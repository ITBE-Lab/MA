...
#include "sequence.h" // sequence slices
 ...
 */
template<class T_scoring,	// must be int16_t for 
...
struct SW_SIMD_Aligner 
{
	...
	// const SimilarityMatrix<T_scoring, T_size_t> xSimilarityMatrix;

	const SmithWatermanParamaterSet<T_scoring> &pSWparameterSetRef;
	 ...
	 */
	static const int scoresPerBlock = sizeof(T__mXXXi) / sizeof(T_scoring);
	...
	{	
		if( sizeof(T_scoring) != 2 )
		{	// The current design works for int16_t only.
			throw AlignerException("Datatype for scores has wrong dimension (Must be int16_t)");
		} // if
		
		if ( SWparameterSet.iPossibleMaxScore(uiQueryLen) > std::numeric_limits<T_scoring>::max() )
		{	// The query might create a maximum beyond the max of T_scoring.
						  ...
						  const SmithWatermanParamaterSet<T_scoring> &SWparameterSet)
	{
		...
		*/
		SimilarityMatrix<T_scoring, T_size_t> xSimilarityMatrix(SWparameterSet.iWeightMatch, SWparameterSet.iWeightMismatch, SWparameterSet.uiAlphabetSize);
		T_scoring *profileReference = (T_scoring*)pAlignedAllocatedMemoryRef;
		
		for (unsigned int alphabetIterator = 0; alphabetIterator < SWparameterSet.uiAlphabetSize; ++alphabetIterator) 
		{
			const T_scoring *scroringTableRow = xSimilarityMatrix.pSimilarityMatrixRef + (alphabetIterator * SWparameterSet.uiAlphabetSize);
			for( T_size_t i = 0; i < uiNumberOfBlocks; ++i ) 
			{
				for( T_size_t k = i; k < uiNumberOfBlocks * scoresPerBlock; k += uiNumberOfBlocks ) // p iterations
				{	/* We take std::numeric_limits<T_scoring>::min() for the "non existing columns".
					 * (Don't take 0 instaed of min, because then we get faulty values.)
					 */
					*profileReference++ = (k >= uiQueryLen ? std::numeric_limits<T_scoring>::min() // non existing column
					 ...
					 ) 
	{
		...
		for( T_size_t uiRow = 0; uiRow < uiLenReference; ++uiRow ) 
		{
			...
			{	/* SW cells are computed in the following order:
				 ...
				 */
				f = _mmXXX_subs_epu16(f, gapePar8); 
			 ...
			 */
			T_scoring iMaxScoreOfCurrentRow;
