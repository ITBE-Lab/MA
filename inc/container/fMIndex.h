...
#define bwt_B0(k) (bwt_bwt(k) >> ((~(k)&0xf) << 1) & 3)
    ...
    void _vInitCountTable()
    {
        for (unsigned int i = 0; i != 256; ++i)
        {
            ...
            {
                x |= (((i & 3) == j) + ((i >> 2 & 3) == j) + ((i >> 4 & 3) == j) + (i >> 6 == j)) << (j << 3); // (j << 3) is 0, 8, 16, 24
            }                                                                                                  // for
    ...
    {
        return _aCountTable[(ux16symbols)&0xff] + _aCountTable[(ux16symbols) >> 8 & 0xff] // parallel add    of 4 bytes
  ...
  public:
    ...
    static bool packExistsOnFileSystem(const std::string &rsPrefix)
    {
        ...
        return fileExists(std::string(rsPrefix).append(".bwt")) && fileExists(std::string(rsPrefix).append(".sa"));
    } // method
