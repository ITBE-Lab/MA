...
template <unsigned int ALPHABET_SIZE = 5> class KswCppParam
{
  public:
    /* Attributes */
    ...
    /* Default constructor */
    KswCppParam( ) : max_sc( 2 ), min_sc( -4 )
    {
        fillScoringMatrix( );
    } // default constructor

    /* Constructor */
    KswCppParam( int8_t iMatchSc, // match score
     ...
     */
    template <typename TP_SCR> bool riskOfOverflow( int64_t iSize ) const
    {
...
void kswcpp_avx2( int qlen, // query length -- TO DO: size_t
                  const uint8_t* query, // query sequence
                  int tlen, // reference length -- TO DO: size_t
                  const uint8_t* target, // reference sequence
#ifdef USE_CPP_PARAM
...
static inline void kswcpp_dispatch( int qlen, // query length -- TO DO: size_t
                                    const uint8_t* query, // query sequence
                                    int tlen, // reference length -- TO DO: size_t
                                    const uint8_t* target, // reference sequence
#ifdef USE_CPP_PARAM
                                    ...
                                    AlignedMemoryManager& rxMemManager )
{
    ...
    dispatchbyCPU<void>( vFlags, kswcpp_avx2, kswcpp_sse_xx, qlen, query, tlen, target,
#ifdef USE_CPP_PARAM
                         xParam,
#else
                         m, mat, q, e, q2, e2,
