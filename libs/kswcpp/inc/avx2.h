...
*/
template<typename TP_SELF>
struct alignas(32) Vec_m256i_base
{
	__m256i val;
	...
	{
#ifdef _MSC_VER
		return   __popcnt64( _mm256_extract_epi64( this->val, 0 ) )
			   + __popcnt64( _mm256_extract_epi64( this->val, 1 ) )
			   + __popcnt64( _mm256_extract_epi64( this->val, 2 ) )
			   + __popcnt64( _mm256_extract_epi64( this->val, 3 ) );
#elif defined (__GNUC__)
	...
	Vc_INTRINSIC TP_SELF operator&( const TP &operand ) const
	{
	...
	template<typename TP>
	Vc_INTRINSIC TP_SELF andnot( const TP &operand ) const
	{
	 ...
	 * See: https://stackoverflow.com/questions/32300441/error-when-calling-an-integral-template-member-function-with-g-and-clang
	 */
	template<unsigned int SHIFT>
	Vc_INTRINSIC TP_SELF shiftl( void ) const
	{
		if ( SHIFT ==  0 ) 
			return static_cast< const TP_SELF &>(*this);
		else if( SHIFT <  16 ) 
			return TP_SELF(_mm256_alignr_epi8(this->val, _mm256_permute2x128_si256(this->val, this->val, _MM_SHUFFLE(0, 0, 2, 0)), (uint8_t)(16 - SHIFT)));
		else if( SHIFT == 16 ) 
			return TP_SELF(_mm256_permute2x128_si256(this->val, this->val, _MM_SHUFFLE(0, 0, 2, 0)));
		else 
			return TP_SELF(_mm256_slli_si256 ( _mm256_permute2x128_si256(this->val, this->val, _MM_SHUFFLE(0, 0, 2, 0)), (uint8_t)(SHIFT - 16)));
	} //method


	/* Byte-wise right shifting with respect to the complete 256-bit vector * If you use this with GCC, you need additionally a template indication!
	 * See: https://stackoverflow.com/questions/32300441/error-when-calling-an-integral-template-member-function-with-g-and-clang
	 */
	template<unsigned int SHIFT>
	Vc_INTRINSIC TP_SELF shiftr( void ) const
	{
		if ( SHIFT ==  0 ) 
			return static_cast<const TP_SELF &>(*this);
		else if( SHIFT <  16 ) 
			return TP_SELF(_mm256_alignr_epi8(_mm256_permute2x128_si256(this->val, this->val, _MM_SHUFFLE(2, 0, 0, 1)), this->val, (uint8_t)(SHIFT)));
		else if( SHIFT == 16 ) 
			return TP_SELF(_mm256_permute2x128_si256(this->val, this->val, _MM_SHUFFLE(2, 0, 0, 1)));
		else 
			return TP_SELF(_mm256_srli_si256 (_mm256_permute2x128_si256(this->val, this->val, _MM_SHUFFLE(2, 0, 0, 1)), (uint8_t)(SHIFT - 16)));
	} //method
...
struct alignas(32) Vec_m256i_32_int8_t : public Vec_m256i_base<Vec_m256i_32_int8_t>
{
	...
	template<typename TP_VEC>
	Vc_INTRINSIC Vec_m256i_32_int8_t max( const TP_VEC &vec ) const
	{
		/* REQUIRES SSE 4.1*/
	...
	/* Vertical maximum with respect to the argument */
	template<typename TP_VEC>
	Vc_INTRINSIC Vec_m256i_32_int8_t min( const TP_VEC &vec ) const
	{
		/* REQUIRES SSE 4.1*/
	...
	template<typename TP>
	Vc_INTRINSIC Vec_m256i_32_int8_t operator+( const TP &operand ) const
	{
...
struct alignas(32) Vec_m256i_8_int32_t : public Vec_m256i_base<Vec_m256i_8_int32_t>
{
	...
	{
		std::cout 
			<< _mm256_extract_epi32( this->val, 0 ) << ", "
			<< _mm256_extract_epi32( this->val, 1 ) << ", "
			<< _mm256_extract_epi32( this->val, 2 ) << ", "
			<< _mm256_extract_epi32( this->val, 3 ) << ", "
			<< _mm256_extract_epi32( this->val, 4 ) << ", "
			<< _mm256_extract_epi32( this->val, 5 ) << ", "
			<< _mm256_extract_epi32( this->val, 6 ) << ", "
			<< _mm256_extract_epi32( this->val, 7 ) << 
			std::endl;
	} // method
	...
	Vc_INTRINSIC Vec_m256i_8_int32_t& set8( const int32_t iVal0, const int32_t iVal1, const int32_t iVal2, const int32_t iVal3,
											const int32_t iVal4, const int32_t iVal5, const int32_t iVal6, const int32_t iVal7 )
	{
	...
	{	/* The below code is an efficient replacement for:
		...
		const __m256i vShuffle = _mm256_setr_epi8( 0, 0x80U, 0x80U, 0x80U, 1, 0x80U, 0x80U, 0x80U, 2, 0x80U, 0x80U, 0x80U, 3, 0x80U, 0x80U, 0x80U,
												   0, 0x80U, 0x80U, 0x80U, 1, 0x80U, 0x80U, 0x80U, 2, 0x80U, 0x80U, 0x80U, 3, 0x80U, 0x80U, 0x80U );
	...
	template<typename TP>
	Vc_INTRINSIC Vec_m256i_8_int32_t operator+( const TP &operand ) const
	{
...
struct alignas(32) Vec_m256i_16_int16_t : public Vec_m256i_base<Vec_m256i_16_int16_t>
{
	...
	{
		std::cout
			<< _mm256_extract_epi16( this->val, 0 ) << ", " << _mm256_extract_epi16( this->val, 1 ) << ", "
	...
	{	/* The below code is an efficient replacement (on Intel processors) for:
		 ...
		 */
		// Get 64 bit (8 times int8_t) from the pointer and store them in the lower lane
		const __m256i vLoadMask = _mm256_setr_epi32( 0xFFFFFFFF, 0xFFFFFFFF, 0xFFFFFFFF, 0xFFFFFFFF, 0x0, 0x0, 0x0, 0x0 );
		...
		// Shuffle the bytes in both lanes to the correct positions
		const __m256i vShuffle = _mm256_setr_epi8( 0, 0x80U, 1, 0x80U, 2, 0x80U, 3, 0x80U, 4, 0x80U, 5, 0x80U, 6, 0x80U, 7, 0x80U,
												   0, 0x80U, 1, 0x80U, 2, 0x80U, 3, 0x80U, 4, 0x80U, 5, 0x80U, 6, 0x80U, 7, 0x80U );
		const __m256i vShuff8Bytes = _mm256_shuffle_epi8( v32Bitx2InBothLanes, vShuffle );
		// if greater 127, it was a negative value (255 = -1, ... , 128 = - 128)
	...
	/* Parallel addition of eight 32 bit values */
	template<typename TP>
	Vc_INTRINSIC Vec_m256i_16_int16_t operator+( const TP &operand ) const
	{
