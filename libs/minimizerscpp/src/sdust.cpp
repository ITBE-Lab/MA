...
{
    kdq_t( int ) * w;
    perf_intv_v P; // the list of perfect intervals for the current window, sorted by descending start and then by
                   // ascending finish
...
{
    int s;
    if( (int)kdq_size( w ) >= W - SD_WLEN + 1 )
    { // TODO: is this right for SD_WLEN!=3?
        s = *kdq_shift( int, w );
        *rw -= --cw[ s ];
        if( *L > (int)kdq_size( w ) )
            --*L, *rv -= --cv[ s ];
...
static inline void save_masked_regions( void* km, uint64_v* res, perf_intv_v* P, int start )
{
    int i, saved = 0;
    perf_intv_t* p;
    if( P->n == 0 || P->a[ P->n - 1 ].start >= start )
        return;
    p = &P->a[ P->n - 1 ];
    if( res->n )
    {
    ...
    if( !saved )
        kv_push( uint64_t, km, *res, (uint64_t)p->start << 32 | p->finish );
    for( i = P->n - 1; i >= 0 && P->a[ i ].start < start; --i )
        ; // remove perfect intervals that have falled out of the window
    P->n = i + 1;
}

static void find_perfect( void* km, perf_intv_v* P, const kdq_t( int ) * w, int T, int start, int L, int rv,
                          const int* cv )
{
    ...
    for( i = (long)kdq_size( w ) - L - 1; i >= 0; --i )
    {
        ...
        if( new_r * 10 > T * new_l )
        {
            for( j = 0; j < (int)P->n && P->a[ j ].start >= i + start; ++j )
            ...
            { // then insert
                ...
                if( P->n == P->m )
                    kv_resize( perf_intv_t, km, *P, P->n + 1 );
...
const uint64_t* sdust_core( const uint8_t* seq, int l_seq, int T, int W, int* n, sdust_buf_t* buf )
{
    ...
    for( i = l = t = 0; i <= l_seq; ++i )
    {
        ...
        { // N or the end of sequence; N effectively breaks input into pieces of independent sequences
