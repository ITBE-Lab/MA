...
static void *worker_pipeline(void *shared, int step, void *in)
{
    ...
    if (step == 0) { // step 0: read sequences
        ...
        step_t *s = (step_t*)in;
		mm_idx_add(p->mi, (int)s->a.n, s->a.a);
...
mm_idx_t *mm_idx_gen(mm_bseq_file_t *fp, int w, int k, int b, int flag, int mini_batch_size, int n_threads, uint64_t batch_size)
{
	...
	pl.mini_batch_size = (int)((uint64_t)mini_batch_size < batch_size? mini_batch_size : batch_size);
...
void mm_idx_dump(FILE *fp, const mm_idx_t *mi)
{
	...
	for (i = 0; i < mi->n_seq; ++i) {
		if (mi->seq[i].name) {
			uint8_t l = (uint8_t)strlen(mi->seq[i].name);
			fwrite(&l, 1, 1, fp);
