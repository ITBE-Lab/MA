    ...
    for( j = 1, n = 1, start_a = start_p = 0; j <= b->a.n; ++j )
    {
        if( j == b->a.n || b->a.a[ j ].x >> 8 != b->a.a[ j - 1 ].x >> 8 )
        {
            ...
            else
            {
                ...
                radix_sort_64(
                    &b->p[ start_p ],
                    &b->p[ start_p + n ] ); // sort by position; needed as in-place radix_sort_128x() is not stable
...
mm_idx_t* mm_idx_gen( mm_bseq_file_t* fp, int w, int k, int b, int flag, int mini_batch_size, int n_threads,
                      uint64_t batch_size )
{
    pipeline_t pl;
...
mm_idx_t* mm_idx_reader_read( mm_idx_reader_t* r, int n_threads )
{
    ...
    if( r->is_idx )
    {
        ...
        if( mi && mm_verbose >= 2 &&
            ( mi->k != r->opt.k || mi->w != r->opt.w || ( mi->flag & MM_I_HPC ) != ( r->opt.flag & MM_I_HPC ) ) )
            fprintf( stderr, "[WARNING]\033[1;31m Indexing parameters (-k, -w or -H) overridden by parameters used in "
                             "the prebuilt index.\033[0m\n" );
    }
    else
        mi = mm_idx_gen( r->fp.seq, r->opt.w, r->opt.k, r->opt.bucket_bits, r->opt.flag, r->opt.mini_batch_size,
                         n_threads, r->opt.batch_size );
    if( mi )
    {
        if( r->fp_out )
