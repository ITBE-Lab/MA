...
template <typename DBCon> class SvCallsFromDb
{
    ...
    // DELETED: 	uint32_t>::Iterator xTableIterator;
  ...
  public:
    ...
    SvCallsFromDb( const ParameterSetManager& rParameters, std::shared_ptr<_SV_DB<DBCon>> pDb, int64_t iSvCallerId )
        : pSelectedSetting( rParameters.getSelected( ) ),
          ...
          xQuerySupport( pDb->pDatabase,
                         ...
                         "JOIN sv_jump_table ON sv_call_support_table.jump_id = sv_jump_table.id "
                         "WHERE sv_call_support_table.call_id = ? " )
    // DELETED: xTableIterator(xQuery.vExecuteAndReturnIterator(iSvCallerId))
    {
        ...
        : pSelectedSetting( rParameters.getSelected( ) ),
          ...
          xQuery( pDb->pDatabase,
                  ( std::string( "SELECT id, from_pos, to_pos, from_size, to_size, switch_strand, inserted_sequence, "
                    ...
                    "     AND idx_inner2.id != inner.id "
                    "     AND " +
                    SvCallTable<DBCon>::getSqlForCallScore( "inner2" ) + " >= " + SvCallTable<DBCon>::getSqlForCallScore( "inner" ) +
                    "     AND idx_inner2.run_id_b >= inner.id " // dim 1
                    "     AND idx_inner2.run_id_a <= inner.id " // dim 1
                    "     AND idx_inner2.maxX >= inner.from_pos - ? " // dim 2
                    "     AND idx_inner2.minX <= inner.from_pos + inner.from_size + ? " // dim 2
     ...
     * @brief fetches all calls of a specific caller id in the rectangle uiX,uiY,uiW,uiH with a score >= dMinScore.
     */
    SvCallsFromDb( const ParameterSetManager& rParameters, std::shared_ptr<_SV_DB<DBCon>> pDb, int64_t iSvCallerId, int64_t iX,
                   int64_t iY, int64_t iW, int64_t iH, double dMinScore )
        : pSelectedSetting( rParameters.getSelected( ) ),
          pDb( pDb ),
          xQuery( pDb->pDatabase,
