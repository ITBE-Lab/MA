...
template <typename DBConnector> void checkDB( std::shared_ptr<SQLDB<DBConnector>> pMySQLDB, int jobnr )
{
...

    {
        ...
        int numValues = 1000000;
        {
            ...
            auto xBulkInserter = xTestTable.template getBulkInserter<10>( );
            std::string text = "This is a string of some size for insertion ...";
            SomeBlobType blob;

            metaMeasureAndLogDuration( "Large Table: Time bulk insert:", [&]( ) {
                for( int i = 0; i < numValues; i++ )
                    xBulkInserter->insert( nullptr, 10, (double)4, blob, text, std::numeric_limits<uint32_t>::max( ) );
            } );
        } // scope transaction
        ...
        for( int32_t i = 1; i <= 10; i++ )
        {
            ...
            for( int j = 0; j < i; j++ )
                text.append( "+" );
            auto val = xTestTable.insert( i + 10, (double)i, blob, text, std::numeric_limits<uint32_t>::max( ) );
