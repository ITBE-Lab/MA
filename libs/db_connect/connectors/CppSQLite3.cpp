...
int sqlite3_decode_binary( const unsigned char* xFilteredInputStreamBuf, unsigned char* out );

////////////////////////////////////////////////////////////////////////////////

////////////////////////////////////////////////////////////////////////////////

CppSQLite3Exception::CppSQLite3Exception( const int nErrCode, const char* szErrMess, bool bDeleteMsg /*=true*/ )
    : mnErrCode( nErrCode )
{
...
/* Executing the statement as non-query.
 */
...
/* Executing the statement as query.
 */
...
/* Inserted by Arne 19/03/2016 due to problems with table buffers.
 */
void CppSQLite3Statement::bind( int nParam, const std::string& szValue )
{
    checkVM( );
...
CppSQLite3Statement CppSQLite3DB::compileStatement( const char* szSQL )
{
    ...
    sqlite3_stmt* pVM = compile_v2( szSQL );
...
sqlite3_stmt* CppSQLite3DB::compile_v2( const char* szSQL )
{
    checkDB( );
...
/*
* Taken from https://www.sqlite.org/backup.html
...
*/
int CppSQLite3DB::loadOrSaveDb( const char* zFilename, int isSave )
{
    ...
    */
    ...
    if( rc == SQLITE_OK )
    {
        ...
        * pTo accordingly. 
        */
        pFrom = ( isSave ? mpDB : pFile );
        pTo = ( isSave ? pFile : mpDB );
...
int sqlite3_encode_binary( const unsigned char* xFilteredInputStreamBuf, int n, unsigned char* out )
{
    ...
    for( i = 0; i < n; i++ )
    {
        int c = ( xFilteredInputStreamBuf[ i ] - e ) & 0xff;
        if( c == 0 )
        {
            out[ j++ ] = 1;
            out[ j++ ] = 1;
...
{
    int i, c, e;
    e = *( xFilteredInputStreamBuf++ );
    i = 0;
    while( ( c = *( xFilteredInputStreamBuf++ ) ) != 0 )
    {
        if( c == 1 )
        {
            c = *( xFilteredInputStreamBuf++ );
            if( c == 1 )
            {
                c = 0;
            }
