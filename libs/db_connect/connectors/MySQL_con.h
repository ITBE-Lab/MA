...
class MySQLConDB
{
  public:
    ...
    template <typename DBPtrType> class PreparedStmtTmpl
    {
      ...
      public:
         ...
         */
        template <typename... ArgTypes> my_ulonglong bindAndExec( ArgTypes&&... args )
        {
            ...
            // Bind all argument
            bindArgumentsForwarding<0, ArgTypes&&...>( std::forward<ArgTypes>( args )... );
              ...
              iStatus( -1 ) // initially unknown
        {
            ...
            if( NUM_COLS != uiColumnCount )
                throw std::runtime_error(
                    "MySQL - Your SQL statement does not match the number of template parameters." );
  ...
  private:
    ...
    void setDB( const std::string& rsDBName )
    {
        checkDBCon( );
