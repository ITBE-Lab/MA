...
// Part1 : Specify the corresponding MySQL-type for your blob.
template <> inline std::string MySQLConDB::TypeTranslator::getSQLTypeName<WKBPoint>( )
{
...
// Part1b : Spatial types require an indication that the argument passed at a placeholder's
//          position has the format 'WKB'.
template <> inline std::string MySQLConDB::TypeTranslator::getPlaceholderForType<WKBPoint>( )
{
    return "ST_PointFromWKB(?, 0)";
} // specialized method

// Part 2: Input arguments: Set the start of the blob (void *), size of the blob and type of the blob.
template <> inline void MySQLConDB::StmtArg::set( const WKBPoint& rRectangle )
{
    this->uiLength = static_cast<unsigned long>( WKBPoint::uiSizeWKB );
    pMySQLBind->buffer_length = static_cast<unsigned long>( WKBPoint::uiSizeWKB );
    pMySQLBind->buffer_type = MYSQL_TYPE_BLOB; // this type must be equal to the type in Part 3.
    pMySQLBind->buffer = rRectangle.getData( );
...
struct /* MySQLConDB:: */ RowCell<WKBPoint> : public /* MySQLConDB::*/ RowCellBase<WKBPoint>
{
    ...
    {
        RowCellBase<WKBPoint>::init( pMySQLBind, pCellValue, MYSQL_TYPE_BLOB, uiColNum );
    } // method
