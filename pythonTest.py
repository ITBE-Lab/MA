...
def test_my_approach(
    ...
    for index, queries, in enumerate(chunks(all_queries, extract_size)):
        ...
        if specific_id != None or specific_query != None:
            if len(pledges[-1][0].get()) == 0:
                print("No alignment Done")
            else:
                for alignment in pledges[-1][0].get()[:3]:
...
def compute_bam_bai_for_files(path_name_gen, task_name, pack, reference, aligners=[0]):
    ...
    for aligner_idx in aligners: #[0, 1, 2, 3]
        ...
        for path, file_name in path_name_gen:
            ...
            aligner_commands = [ma_cmd, mm_cmd, ngmlr_cmd, ma_old_cmd]
            #os.system(ma_cmd)
            #os.system(mm_cmd)
            #os.system(ngmlr_cmd)
            os.system(aligner_commands[aligner_idx])
...
def extract_section_from_bam(bam_in, fasta_out, chr, start, end):
    cmd = "samtools view " + bam_in + " " + chr + ":" + str(start) + "-" + str(end)
    cmd += """ | awk '{OFS="\\t"; print ">"$1"\\n"$10}' - > """ + fasta_out
    print("extracting")
    os.system(cmd)
...
sons_folder = "/mnt/ssd0/PacBio/ftp-trace.ncbi.nlm.nih.gov/giab/ftp/data/AshkenazimTrio/HG002_NA24385_son/PacBio_MtSinai_NIST/Baylor_NGMLR_bam_GRCh37/"

#extract_section_from_bam(
#        sons_folder + "HG002_PB_70x_RG_HP10XtrioRTG.bam",
#        sons_folder + "gen_dup_section.fasta",
#        "17",
#        44360234,
#        44686464
#    )
#filter_duplicates(
#        sons_folder + "gen_dup_section.fasta",
#        sons_folder + "gen_dup_section_filterd.fasta"
#    )
#compute_bam_bai_for_files(
#        [ ( sons_folder, "gen_dup_section_filterd.fasta") ],
#        "gen_dup",
...
#test_my_approaches("None", "/MAdata/genome/GRCh38.p12", specific_query="")
#exit()
