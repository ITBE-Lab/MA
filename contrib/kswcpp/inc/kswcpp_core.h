...
static inline void ksw_backtrack__(void *km, int is_rot, int is_rev, int min_intron_len, const TP_DIFF_SCORE_UNSIGNED *p, const int *off, const int *off_end, int n_col, int i0, int j0,
...
static inline bool calcMaxScore(
	const int &approx_max, // boolean switch
	const int &r, // row
	...
	if ( !approx_max ) { // find the exact max with a 32-bit score array
		...
		{	
			...
			{	// this implements: H[t]+=v8[t]-qe; if(H[t]>max_H) max_H=H[t],max_t=t;
				...
				v_H1.load( &H[t] );
				T_SIMD_VEC v_t_ = T_SIMD_VEC::expand_int8_t( &v8[t] ); // v_t_.set4( v8[t], v8[t+1], v8[t+2], v8[t+3] );
				v_H1 = v_H1 + v_t_;
				v_H1.store( &H[t] );
				v_t_.set1( t );
				v_tmp = v_H1 > v_max_H_;
...
template<
	typename TP_SCORE, // type for scoring
...
>
static inline void kswcpp_inner_core(
	...
	AlignedMemoryManager &rxMemManager )
{
	...
	int t, qe = q + e, n_col_, *off = 0, *off_end = 0, qlen_, /* last_st, */ last_en, 
		wl, wr, max_sc, min_sc, long_thres, long_diff;
	...
	{	/* Allocate aligned memory for p */
		...
		p_pstruct = (TP_DIFF_VEC*)((((size_t)mem2 + (sizeof(TP_DIFF_VEC) - 1)) / sizeof(TP_DIFF_VEC)) * sizeof(TP_DIFF_VEC));
	...
	for( int r = 0, last_st = last_en = -1; r < qlen + tlen - 1; ++r ) 
	{
		...
		{	/* Use the matrix values for the scoring profile */
			for (t = st0; t <= en0; ++t)
				((TP_DIFF_SCORE_UNSIGNED*)s_pstruct)[t] = mat[sf[t] * m + qrr[t]]; // strange; here is cast to unsigned element type (uint8_t)
		} // else
		...
		bool bStop = calcMaxScore<TP_SCORE_VEC>
			( approx_max, // approx_max, // boolean switch
			  r, // row
	...
	{	/* Cigar construction by backtracking */
		...
		{	/* Backtracking up to tlen - 1, qlen - 1 */
			ksw_backtrack__( nullptr, 1, rev_cigar, 0, (TP_DIFF_SCORE_UNSIGNED*)p_pstruct,
							 off, off_end, n_col_*TP_DIFF_VEC::SIZE, tlen - 1, qlen - 1,
		...
		{	/* Backtracking up to mqe_t, qlen - 1 */
			ez->reach_end = 1;
			ksw_backtrack__( nullptr, 1, rev_cigar, 0, (TP_DIFF_SCORE_UNSIGNED*)p_pstruct,
							 off, off_end, n_col_*TP_DIFF_VEC::SIZE, ez->mqe_t, qlen - 1,
							 &ez->m_cigar, &ez->n_cigar, &ez->cigar );
		} // else if
		else if( ez->max_t >= 0 && ez->max_q >= 0 )
		{	/* Backtracking up to max_t, max_q */
			ksw_backtrack__( nullptr, 1, rev_cigar, 0, (TP_DIFF_SCORE_UNSIGNED*)p_pstruct,
							 off, off_end, n_col_*TP_DIFF_VEC::SIZE, ez->max_t, ez->max_q,
							 &ez->m_cigar, &ez->n_cigar, &ez->cigar );
		} // else if
		kfree( nullptr, mem2 ); kfree( nullptr, off );
	} // if
} // function
	...
	AlignedMemoryManager &rxMemManager )
{
	...
	if( bWithCigar )
		if( bLeftGapAlign )
			kswcpp_inner_core
				<TP_SCORE, TP_SCORE_VEC, TP_DIFF_SCORE, TP_DIFF_SCORE_UNSIGNED, TP_DIFF_VEC, true, true>
				( qlen, query,  tlen, target, xParam, w, zdrop, flag, ez, rxMemManager );
		else
			kswcpp_inner_core
				<TP_SCORE, TP_SCORE_VEC, TP_DIFF_SCORE, TP_DIFF_SCORE_UNSIGNED, TP_DIFF_VEC, false, true>
				( qlen, query,  tlen, target, xParam, w, zdrop, flag, ez, rxMemManager );
	else
		/* Without cigar implies that the gap alignments is not significant */
		kswcpp_inner_core
			<TP_SCORE, TP_SCORE_VEC, TP_DIFF_SCORE, TP_DIFF_SCORE_UNSIGNED, TP_DIFF_VEC, true, false>
			( qlen, query,  tlen, target, xParam, w, zdrop, flag, ez, rxMemManager );
} // function
