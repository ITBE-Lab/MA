...
void exportSoC()
{
#if DEBUG_LEVEL >= 1
    boost::python::class_<
        std::pair<nucSeqIndex, nucSeqIndex>>("nucSeqPair", boost::python::init<nucSeqIndex, nucSeqIndex>())
    ...
    boost::python::class_<std::vector<std::pair<nucSeqIndex, nucSeqIndex>>>("nucSeqPairVector")
        .def(boost::python::vector_indexing_suite<
             ...
             true>());
    boost::python::class_<
        SoCPriorityQueue::blub>("nucSeqnucSeqInt")
    ...
    boost::python::class_<std::vector<SoCPriorityQueue::blub>>("nucSeqnucSeqIntVector")
        .def(boost::python::vector_indexing_suite<
              ...
              * true = noproxy this means that the content of the vector is already exposed by
              * boost python. 
              * if this is kept as false, SoCPriorityQueue would be exposed a second time.
              * the two SoCPriorityQueues would be different and not inter castable.
              */
             true>());
    boost::python::class_<std::vector<std::shared_ptr<Seeds>>>("seedVector")
        .def(boost::python::vector_indexing_suite<
             ...
             /*
              * true = noproxy this means that the content of the vector is already exposed by 
              * boost python. If this is kept as false, Seeds would be exposed a second time. 
              * The two Seeds would be different and not inter castable.
              */
             true>());
    ...
    boost::python::class_<
        ...
        std::shared_ptr<SoCPriorityQueue>>("SoCPriorityQueue")
        .def(
            "empty",
