...
float EXPORTED fGiveUp = 0.002; // do not store socs with score less than this * query len
float EXPORTED fRelMinSeedSizeAmount = 0.005; // minimum seed coverage to consider a query
float EXPORTED fScoreDiffTolerance = 0.0001; // break if the harm score falls faster than this
float EXPORTED fSoCScoreMinimum = 0; // minimum score used for the SoC width computation
float EXPORTED fMinimalQueryCoverage = 1.1; // does nothing at the moment @todo
float EXPORTED fScoreTolerace = 0.1; // break if the SoC score drops faster than this
