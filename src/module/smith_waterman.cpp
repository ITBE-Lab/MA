...
{
    return ContainerVector{// the query sequence
 ...
 */
int16_t alignSW_SIMD(
    NucSeq &rQuerySequence, // query sequence
    ...
    std::vector<size_t>
        &rvMaxScorePositions // vector will recieve positions, where we have a max score
)
{
...
std::shared_ptr<Container> SMW::execute( std::shared_ptr<ContainerVector> vpInput )
{
        ...
        DEBUG( if ( ( uiMaxScore != pAlignment->score( ) || bPrint ) ) {
            ...
            if ( best != uiIndex )
                std::cout << "row: " << vMaxScorePositions[ 0 ].first
                          ...
                          << ", cols: " << xAligner.numberOfColumns << ", should: " << best
                          << ", is: " << uiIndex << std::endl;
                ...
                std::cout
                    ...
                    << ( xAligner.alignmentOutcomeMatrix.backtrackMatrix[ 0 ][ i ] == LEFT
                             ...
                             : ( xAligner.alignmentOutcomeMatrix.backtrackMatrix[ 0 ][ i ] == UP
                                     ? "U"
                                     : ( xAligner.alignmentOutcomeMatrix
                                                     .backtrackMatrix[ 0 ][ i ] == STOP
                                             ? "S"
                                             : "D" ) ) )
                    << ","
    ...
    else
    {
        ...
        auto imaxScore = alignSW_SIMD(
            *pQuerySeq, // query sequence
        ...
        for ( nucSeqIndex revStart : vMaxScorePositions )
        {
            ...
            std::shared_ptr<Alignment> pAlignment =
                std::shared_ptr<Alignment>( new Alignment( pReference->length( ) - revStart ) );
...
void exportSMW( )
{
    boost::python::class_<SMW, boost::python::bases<Module>, std::shared_ptr<SMW>>(
        "SMW", boost::python::init<bool>( ) )
        DEBUG(.def_readwrite( "print", &SMW::bPrint ) ) // DEBUG
        ;
    boost::python::implicitly_convertible<std::shared_ptr<SMW>, std::shared_ptr<Module>>( );
} // function
#endif