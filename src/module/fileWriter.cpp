...
ContainerVector FileWriter::getInputType( ) const
{
    return ContainerVector{
        std::shared_ptr<NucSeq>( new NucSeq( ) ), std::shared_ptr<NucSeq>( new NucSeq( ) ),
...
std::shared_ptr<Container> FileWriter::execute( std::shared_ptr<ContainerVector> vpInput )
{
    ...
    std::shared_ptr<NucSeq> pQuery2 = std::dynamic_pointer_cast<NucSeq>( ( *vpInput )[ 1 ] ); // dc
    ...
    for ( std::shared_ptr<Container> pA : *pAlignments )
    {
        ...
        DEBUG( // check if the position that is saved to the file is correct
            ...
            if ( pPack->bPositionIsOnReversStrand( pAlignment->uiBeginOnRef ) ) {
                ...
                if ( pAlignment->uiBeginOnRef !=
                     pPack->startOfSequenceWithName( sRefName ) + uiRefPos - 1 )
                    bWrong = true;
            } // else
        ...
        if ( std::isnan( pAlignment->fMappingQuality ) )
            ...
            sMapQual = std::to_string(
                static_cast<int>( std::ceil( pAlignment->fMappingQuality * 254 ) ) );
...
std::shared_ptr<Container> RadableFileWriter::execute( std::shared_ptr<ContainerVector> vpInput )
{
    ...
    for ( std::shared_ptr<Container> pA : *pAlignments )
    {
        ...
        if ( !pAlignment->xStats.pOther.expired( ) )
            sPaired =
                pAlignment->xStats.bFirst ? "first mate of read pair" : "second mate of read pair";

        std::string sRefName = pPack->nameOfSequenceWithId(
            pPack->uiSequenceIdForPosition( pAlignment->uiBeginOnRef ) );
        // 1 based index...
        std::string sRefPos =
            std::to_string( 1 + pAlignment->uiBeginOnRef -
                            pPack->startOfSequenceWithId(
                                pPack->uiSequenceIdForPosition( pAlignment->uiBeginOnRef ) ) );
        std::string sSegmentQuery =
            pQuery->fromTo( pAlignment->uiBeginOnQuery, pAlignment->uiEndOnQuery );
        std::string sQueryPos = std::to_string( pAlignment->uiBeginOnQuery );
        std::string sSegmentRef =
            pPack->vExtract( pAlignment->uiBeginOnRef, pAlignment->uiEndOnRef )->toString( );
        // std::string sQual = pQuery->fromToQual(pAlignment->uiBeginOnQuery,
        // pAlignment->uiEndOnQuery);
        std::string sMapQual;
        if ( std::isnan( pAlignment->fMappingQuality ) )
...

        {
            ...
            *pOut << "Score: " << std::to_string( pAlignment->score( ) )
                  ...
                  << ( pAlignment->bSecondary ? " Secondary\n" : "\n" );
...
std::shared_ptr<Container> SeedSetFileWriter::execute( std::shared_ptr<ContainerVector> vpInput )
{
    ...
    // sort descending
    std::sort( pSoCs->begin( ), pSoCs->end( ),
    ...
    for ( std::shared_ptr<Container> pS : *pSoCs )
    {
        ...
        std::string sOnRevComp =
            ( pPack->bPositionIsOnReversStrand( beginRef ) ? "true" : "false" );
