...
#ifdef __SSE4_1__
void ksw_extd2_sse41( void *km, int qlen, const uint8_t *query, int tlen, const uint8_t *target,
                      int8_t m, const int8_t *mat, int8_t q, int8_t e, int8_t q2, int8_t e2, int w,
                      int zdrop, int flag, ksw_extz_t *ez )
#else
void ksw_extd2_sse2( void *km, int qlen, const uint8_t *query, int tlen, const uint8_t *target,
                     int8_t m, const int8_t *mat, int8_t q, int8_t e, int8_t q2, int8_t e2, int w,
                     int zdrop, int flag, ksw_extz_t *ez )
#endif
#else
...
#define __dp_code_block2                                                                           \
    ...
    for ( r = 0, last_st = last_en = -1; r < qlen + tlen - 1; ++r )
    {
...
#else
                    max_H_ =
                        _mm_or_si128( _mm_and_si128( tmp, H1 ), _mm_andnot_si128( tmp, max_H_ ) );
                    max_t_ =
                        _mm_or_si128( _mm_and_si128( tmp, t_ ), _mm_andnot_si128( tmp, max_t_ ) );
#endif
                }
        ...
        // ((int8_t*)u)[t], ((int8_t*)v)[t], ((int8_t*)x)[t], ((int8_t*)y)[t], H[t]); // for
        // debugging
    }
    kfree( km, mem );
    if ( !approx_max )
