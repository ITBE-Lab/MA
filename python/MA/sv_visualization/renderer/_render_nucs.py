...
def render_nucs(self):
    ...
    nuc_seq = self.pack.extract_from_to(max(int(self.ys - self.h), 0), int(self.ye + self.h + 1))
    ...
    self.d_read_plot.rect(x="y", y=0.5, width=1, height=1, fill_color="c", line_width=0,
                        source=ColumnDataSource(l_plot_nucs), name="hover4")
    d_plot_nucs = {"x": [], "c": [], "i": []}
    nuc_seq = self.pack.extract_from_to(max(int(self.xs - self.w), 0), int(self.xe + self.w + 1))
