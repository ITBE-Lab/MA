...
#define _gaba_parse_ptr( _p )                                                                      \
    ( ( uint64_t const * )( ( uint64_t )( _p ) & ~( sizeof( uint64_t ) - 1 ) ) )
 ...
 * @fn gaba_parse_u64
 */
static inline uint64_t gaba_parse_u64( uint64_t const *ptr, int64_t pos )
{
    int64_t rem = pos & 63;
 ...
 */
static inline uint64_t gaba_parse_dump_num( char *buf, uint64_t len, char ch )
{
    if ( len < 64 )
    {
    ...
    else
    {
        ...
        for ( p -= ( p != &b[ 1 ] ), adv = ( uint64_t )( ( ptrdiff_t )( p - b ) ) + 1; p >= b; p-- )
        {
            *buf++ = *p;
        }
        return ( adv );
...
#define _rv( c )                                                                                   \
    {                                                                                              \
        ...
        {                                                                                          \
            ...
            _storeu_v16i8( r - i,                                                                  \
                           _shuf_v16i8( cv, _swapn_v16i8( _loadu_v16i8( q + i - l ), l ) ) );      \
        }                                                                                          \
    }
#define _nop( c )                                                                                  \
    {                                                                                              \
    ...
    v16i8_t const cv =
        _load_v16i8( ( conf & GABA_SEQ_RV ) ? gaba_parse_ascii_rv : gaba_parse_ascii_fw );
    ...
    switch ( conf )
    {
        case GABA_SEQ_A | GABA_SEQ_FW:
            _parser_loop_fw( _fw, _gap, _fw, _nop );
            break;
        case GABA_SEQ_A | GABA_SEQ_RV:
            _parser_loop_fw( _rv, _gap, _rv, _nop );
            break;
        case GABA_SEQ_B | GABA_SEQ_FW:
            _parser_loop_fw( _gap, _fw, _fw, _nop );
            break;
        case GABA_SEQ_B | GABA_SEQ_RV:
            _parser_loop_fw( _gap, _rv, _rv, _nop );
            break;
    }
    *r = '\0';
    return ( r - buf );
...
#define _nop( c )                                                                                  \
    ...
    {
        case GABA_SEQ_A | GABA_SEQ_FW:
            _parser_loop_rv( _fw, _gap, _fw, _nop );
            break;
        case GABA_SEQ_A | GABA_SEQ_RV:
            _parser_loop_rv( _rv, _gap, _rv, _nop );
            break;
        case GABA_SEQ_B | GABA_SEQ_FW:
            _parser_loop_rv( _gap, _fw, _fw, _nop );
            break;
        case GABA_SEQ_B | GABA_SEQ_RV:
            _parser_loop_rv( _gap, _rv, _rv, _nop );
            break;
        default:
            break;
    }
                            ...
                            gaba_path_section_t const *s, gaba_section_t const *a )
{
    return ( gaba_dump_seq_forward(
        buf, buf_size, GABA_SEQ_A | ( a->base < GABA_EOU ? GABA_SEQ_FW : GABA_SEQ_RV ), path,
        s->ppos, gaba_plen( s ),
...
uint64_t gaba_dump_seq_query( char *buf, uint64_t buf_size, uint32_t const *path,
                              gaba_path_section_t const *s, gaba_section_t const *b )
{
    return ( gaba_dump_seq_forward(
        buf, buf_size, GABA_SEQ_B | ( b->base < GABA_EOU ? GABA_SEQ_FW : GABA_SEQ_RV ), path,
        s->ppos, gaba_plen( s ),
