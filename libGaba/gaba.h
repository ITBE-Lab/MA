...
#define _GABA_WRAP_EXPORT_LEVEL
    ...
    {
        GABA_CONT =
            0, /* continue, call again the function with the same args (but rarely occurrs) */
        GABA_UPDATE_A =
            0x000f, /* update required on section a (always combined with GABA_UPDATE) */
        GABA_UPDATE_B =
            0x00f0, /* update required on section b (always combined with GABA_UPDATE) */
    ...
    struct gaba_params_s
    {
        ...
        int8_t gfa, gfb; /** linear-gap extension penalty for short indels (combined-gap penalty; gf
                            > ge). gfa for gaps on sequence A, gfb for seq. B. */
 ...
 */
#define GABA_SCORE_SIMPLE( _m, _x, _gi, _ge )                                                      \
    .score_matrix = {( _m ),  -( _x ), -( _x ), -( _x ), -( _x ), ( _m ),  -( _x ), -( _x ),       \
                     -( _x ), -( _x ), ( _m ),  -( _x ), -( _x ), -( _x ), -( _x ), ( _m )},       \
    .gi = ( _gi ), .ge = ( _ge )
...
#define gaba_build_section( _id, _base, _len )                                                     \
    ( ( struct gaba_section_s ){                                                                   \
        .id = ( _id ), .base = ( uint8_t const * )( _base ), .len = ( _len )} )
/**
 * @macro GABA_EOU
...
#define gaba_mirror( base, len )                                                                   \
    ( GABA_EOU + ( uint64_t )GABA_EOU - ( uint64_t )( base ) - ( uint64_t )( len ) )
...
#define gaba_plen( seg ) ( ( seg )->alen + ( seg )->blen )
    ...
    struct gaba_alignment_s
    {
        ...
        int64_t score; /** score */
        double identity; /** estimated percent identity over the entire alignment, match_count /
     ...
     */
    _GABA_EXPORT_LEVEL
    void gaba_dp_flush( gaba_dp_t *dp );

    /**
     * @fn gaba_dp_save_stack
     */
    _GABA_EXPORT_LEVEL
    gaba_stack_t const *gaba_dp_save_stack( gaba_dp_t *dp );

    /**
     * @fn gaba_dp_flush_stack
     */
    _GABA_EXPORT_LEVEL
    void gaba_dp_flush_stack( gaba_dp_t *dp, gaba_stack_t const *stack );

    /**
     * @fn gaba_dp_clean
     */
    _GABA_EXPORT_LEVEL
    void gaba_dp_clean( gaba_dp_t *dp );

    /**
     * @fn gaba_dp_fill_root
     */
    _GABA_EXPORT_LEVEL
    gaba_fill_t *gaba_dp_fill_root( gaba_dp_t *dp, gaba_section_t const *a, uint32_t apos,
     ...
     */
    _GABA_EXPORT_LEVEL
    gaba_fill_t *gaba_dp_fill( gaba_dp_t *dp, gaba_fill_t const *prev_sec, gaba_section_t const *a,
...
#define MAX_MERGE_COUNT ( 14 )
    _GABA_EXPORT_LEVEL
    gaba_fill_t *gaba_dp_merge( gaba_dp_t *dp, gaba_fill_t const *const *sec, uint8_t const *qofs,
                                uint32_t cnt );
     ...
     */
    _GABA_EXPORT_LEVEL
    gaba_pos_pair_t *gaba_dp_search_max( gaba_dp_t *dp, gaba_fill_t const *sec );

    /**
     * @fn gaba_dp_trace
     * @brief generate alignment result string, alloc->malloc and alloc->free must not be NULL if
     * alloc is not NULL.
     */
    _GABA_EXPORT_LEVEL
    gaba_alignment_t *gaba_dp_trace( gaba_dp_t *dp, gaba_fill_t const *tail,
     ...
     */
    _GABA_EXPORT_LEVEL
    void gaba_dp_res_free( gaba_dp_t *dp, gaba_alignment_t *aln );
     ...
     */
    _GABA_EXPORT_LEVEL
    gaba_score_t *gaba_dp_calc_score( gaba_dp_t *dp, uint32_t const *path,
                                      gaba_path_section_t const *s, gaba_section_t const *a,
...
#define _GABA_PRINTER_T_DEFINED
     ...
     * @brief dump CIGAR string (4M1I5M1D...) for a range specified by [offset, offset + len) on the
     * path, the range can be retireved from segment by [seg[i].ppos, gaba_plen(&seg[i])).
     */
    _GABA_PARSE_EXPORT_LEVEL
    uint64_t gaba_print_cigar_forward( gaba_printer_t printer, void *fp, uint32_t const *path,
                                       uint64_t offset, uint64_t len );
    _GABA_PARSE_EXPORT_LEVEL
    uint64_t gaba_print_cigar_reverse( gaba_printer_t printer, void *fp, uint32_t const *path,
                                       uint64_t offset, uint64_t len );
     ...
     */
    _GABA_PARSE_EXPORT_LEVEL
    uint64_t gaba_dump_cigar_forward( char *buf, uint64_t buf_size, uint32_t const *path,
                                      uint64_t offset, uint64_t len );
    _GABA_PARSE_EXPORT_LEVEL
    uint64_t gaba_dump_cigar_reverse( char *buf, uint64_t buf_size, uint32_t const *path,
                                      uint64_t offset, uint64_t len );
     ...
     */
    _GABA_PARSE_EXPORT_LEVEL
    uint64_t gaba_dump_seq_forward( char *buf, uint64_t buf_size,
                                    uint32_t conf, /* { SEQ_A, SEQ_B } x { SEQ_FW, SEQ_RV } */
                                    uint32_t const *path, uint64_t offset, uint64_t len,
                                    uint8_t const *seq, /* a->seq[s->alen] when SEQ_RV */
                                    char gap ); /* gap char, '-' */
    _GABA_PARSE_EXPORT_LEVEL
    uint64_t gaba_dump_seq_reverse( char *buf, uint64_t buf_size,
                                    uint32_t conf, /* { SEQ_A, SEQ_B } x { SEQ_FW, SEQ_RV } */
                                    uint32_t const *path, uint64_t offset, uint64_t len,
                                    uint8_t const *seq, /* a->seq[s->alen] when SEQ_RV */
                                    char gap ); /* gap char, '-' */
     ...
     */
    _GABA_PARSE_EXPORT_LEVEL
    uint64_t gaba_dump_seq_ref( char *buf, uint64_t buf_size, uint32_t const *path,
                                gaba_path_section_t const *s, gaba_section_t const *a );
    _GABA_PARSE_EXPORT_LEVEL
    uint64_t gaba_dump_seq_query( char *buf, uint64_t buf_size, uint32_t const *path,
                                  gaba_path_section_t const *s, gaba_section_t const *a );
