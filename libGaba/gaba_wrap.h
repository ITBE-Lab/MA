...
{
    /* fill-in */
    gaba_fill_t *( *dp_fill_root )( gaba_dp_t *self, gaba_section_t const *a, uint32_t apos,
                                    gaba_section_t const *b, uint32_t bpos, uint32_t pridx );
    gaba_fill_t *( *dp_fill )( gaba_dp_t *self, gaba_fill_t const *prev_sec,
                               gaba_section_t const *a, gaba_section_t const *b, uint32_t pridx );

    /* merge two sections */
    gaba_fill_t *( *dp_merge )( gaba_dp_t *dp, gaba_fill_t const *const *sec, uint8_t const *qofs,
                                uint32_t cnt );

    /* max score and pos search */
    gaba_pos_pair_t *( *dp_search_max )( gaba_dp_t *self, gaba_fill_t const *sec );

    /* trace */
    gaba_alignment_t *( *dp_trace )( gaba_dp_t *self, gaba_fill_t const *tail,
                                     gaba_alloc_t const *alloc );

    void *unused[ 3 ];
};
_static_assert( sizeof( struct gaba_api_s ) ==
                8 * sizeof( void * ) ); /* must be consistent to gaba_opaque_s */
...
_decl( gaba_fill_t *, gaba_dp_fill_root, gaba_dp_t *self, gaba_section_t const *a, uint32_t apos,
       gaba_section_t const *b, uint32_t bpos, uint32_t pridx );
_decl( gaba_fill_t *, gaba_dp_fill, gaba_dp_t *self, gaba_fill_t const *prev_sec,
       gaba_section_t const *a, gaba_section_t const *b, uint32_t pridx );
_decl( gaba_fill_t *, gaba_dp_merge, gaba_dp_t *self, gaba_fill_t const *const *sec,
       uint8_t const *qofs, uint32_t cnt );
_decl( gaba_pos_pair_t *, gaba_dp_search_max, gaba_dp_t *self, gaba_fill_t const *sec );
_decl( gaba_alignment_t *, gaba_dp_trace, gaba_dp_t *self, gaba_fill_t const *tail,
       gaba_alloc_t const *alloc );
_decl( void, gaba_dp_res_free, gaba_dp_t *dp, gaba_alignment_t *res );
_decl( gaba_score_t *, gaba_dp_calc_score, gaba_dp_t *dp, uint32_t const *path,
       gaba_path_section_t const *s, gaba_section_t const *a, gaba_section_t const *b );
// _decl(int64_t, gaba_dp_print_cigar_forward, gaba_dp_printer_t printer, void *fp, uint32_t const
// *path, uint32_t offset, uint32_t len); _decl(int64_t, gaba_dp_print_cigar_reverse,
// gaba_dp_printer_t printer, void *fp, uint32_t const *path, uint32_t offset, uint32_t len);
// _decl(int64_t, gaba_dp_dump_cigar_forward, char *buf, uint64_t buf_size, uint32_t const *path,
// uint32_t offset, uint32_t len); _decl(int64_t, gaba_dp_dump_cigar_reverse, char *buf, uint64_t
// buf_size, uint32_t const *path, uint32_t offset, uint32_t len);
...
static struct gaba_api_s const api_table[][ GABA_DP_CTX_MAX ] __attribute__( ( aligned( 32 ) ) ) = {
 ...
 */
static inline int64_t gaba_init_get_index( struct gaba_params_s const *params )
{
    if ( params == NULL )
    {
        return ( GABA_AFFINE );
 ...
 * @fn gaba_set_api
 */
static inline void *gaba_set_api( void *ctx, struct gaba_api_s const ( *api )[ GABA_DP_CTX_MAX ] )
{
    if ( ctx == NULL )
    {
        return ( NULL );
    ...
    {
        _memcpy_blk_aa( &dst[ i ], &( ( struct gaba_api_s const * )api )[ i ],
                        sizeof( struct gaba_api_s ) );
    }
    return ( ( void * )dst );
}
...
gaba_t *gaba_init( gaba_params_t const *params )
{
    ...
    static gaba_t *( *const init_table[][ GABA_DP_CTX_MAX ] )( gaba_params_t const *params ) = {
        {_import( gaba_init_linear_64 ), _import( gaba_init_linear_32 ),
         _import( gaba_init_linear_16 )},
        {_import( gaba_init_affine_64 ), _import( gaba_init_affine_32 ),
         _import( gaba_init_affine_16 )},
#ifndef DISABLE_COMBINED
        {_import( gaba_init_combined_64 ), _import( gaba_init_combined_32 ),
         _import( gaba_init_combined_16 )}
#endif
    };
...
{
    return ( ( gaba_dp_t * )gaba_set_api( ( void * )_import( gaba_dp_init_linear_64 )( ctx ),
                                          _api_array( ctx ) ) );
}
 ...
 */
_GABA_WRAP_EXPORT_LEVEL
void gaba_dp_flush( gaba_dp_t *self )
{
 ...
 */
_GABA_WRAP_EXPORT_LEVEL
gaba_stack_t const *gaba_dp_save_stack( gaba_dp_t *self )
{
 ...
 */
_GABA_WRAP_EXPORT_LEVEL
void gaba_dp_flush_stack( gaba_dp_t *self, gaba_stack_t const *stack )
{
 ...
 */
_GABA_WRAP_EXPORT_LEVEL
void gaba_dp_clean( gaba_dp_t *self )
{
 ...
 */
_GABA_WRAP_EXPORT_LEVEL
gaba_fill_t *gaba_dp_fill_root( gaba_dp_t *self, gaba_section_t const *a, uint32_t apos,
                                gaba_section_t const *b, uint32_t bpos, uint32_t pridx )
{
 ...
 */
_GABA_WRAP_EXPORT_LEVEL
gaba_fill_t *gaba_dp_fill( gaba_dp_t *self, gaba_fill_t const *prev_sec, gaba_section_t const *a,
                           gaba_section_t const *b, uint32_t pridx )
{
 ...
 */
_GABA_WRAP_EXPORT_LEVEL
gaba_fill_t *gaba_dp_merge( gaba_dp_t *self, gaba_fill_t const *const *sec, uint8_t const *qofs,
                            uint32_t cnt )
{
 ...
 */
_GABA_WRAP_EXPORT_LEVEL
gaba_pos_pair_t *gaba_dp_search_max( gaba_dp_t *self, gaba_fill_t const *sec )
{
 ...
 */
_GABA_WRAP_EXPORT_LEVEL
gaba_alignment_t *gaba_dp_trace( gaba_dp_t *self, gaba_fill_t const *tail,
                                 gaba_alloc_t const *alloc )
{
 ...
 */
_GABA_WRAP_EXPORT_LEVEL
void gaba_dp_res_free( gaba_dp_t *dp, gaba_alignment_t *res )
{
 ...
 */
_GABA_WRAP_EXPORT_LEVEL
gaba_score_t *gaba_dp_calc_score( gaba_dp_t *dp, uint32_t const *path, gaba_path_section_t const *s,
                                  gaba_section_t const *a, gaba_section_t const *b )
{
 ...
 */
static inline uint8_t unittest_encode_base( char c )
{
...
/* conversion tables */
    ...
    static uint8_t const table[] = {
        [_b( 'A' )] = A, [_b( 'C' )] = C, [_b( 'G' )] = G,
        [_b( 'T' )] = T, [_b( 'U' )] = T, [_b( 'N' )] = A, /* treat 'N' as 'A' */
        [_b( '_' )] = 0 /* sentinel */
    };
#else /* BIT == 4 */
    enum bases
    ...
    static uint8_t const table[] = {
        [_b( 'A' )] = A,         [_b( 'C' )] = C,         [_b( 'G' )] = G,
        [_b( 'T' )] = T,         [_b( 'U' )] = T,         [_b( 'R' )] = A | G,
        [_b( 'Y' )] = C | T,     [_b( 'S' )] = G | C,     [_b( 'W' )] = A | T,
        [_b( 'K' )] = G | T,     [_b( 'M' )] = A | C,     [_b( 'B' )] = C | G | T,
        [_b( 'D' )] = A | G | T, [_b( 'H' )] = A | C | T, [_b( 'V' )] = A | C | G,
 ...
 */
static void *unittest_build_seqs( void *params )
{
    ...
    uint64_t margin = 64;
    struct unittest_sections_s *sec =
        malloc( sizeof( struct unittest_sections_s ) + ( atot + 1 ) + ( btot + 1 ) + margin );
 ...
 */
static void unittest_clean_seqs( void *ctx )
{
    free( ctx );
}
...
#define with_seq_pair( _a, _b )                                                                    \
    .params = unittest_seq_pair( _a, _b ), .init = unittest_build_seqs, .clean = unittest_clean_seqs

#define omajinai( ) struct unittest_sections_s const *s = ( struct unittest_sections_s const * )ctx;
