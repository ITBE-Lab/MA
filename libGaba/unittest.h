...
#define utkv_init( v )                                                                             \
    ( ( v ).n = 0, ( v ).m = UNITTEST_KV_INIT,                                                     \
      ( v ).a = ( __typeof__( ( v ).a ) )calloc( ( v ).m, sizeof( *( v ).a ) ) )
#define utkv_destroy( v )                                                                          \
    {                                                                                              \
        free( ( v ).a );                                                                           \
        ( v ).a = NULL;                                                                            \
    }
...
{
    /* printers */
    void ( *failed )( struct ut_s const *info, struct ut_global_config_s const *gconf,
                      struct ut_group_config_s const *config, int64_t line, char const *func,
                      char const *expr, char const *fmt, ... );

    void ( *result )( struct ut_global_config_s const *gconf,
                      struct ut_group_config_s const *config, struct ut_result_s const *result,
                      int64_t file_cnt );
};
...
struct ut_s
{
    ...
    /* per-function config */
    void ( *fn )( void *ctx, void *gctx, struct ut_s *info,
...
ut_static_assert( offsetof( struct ut_group_config_s, unique_id ) ==
                  offsetof( struct ut_s, unique_id ) );
...
ut_static_assert( offsetof( struct ut_group_config_s, depends_on ) ==
                  offsetof( struct ut_s, depends_on ) );
...
#define UNITTEST_ARG_DECL                                                                          \
    void *ctx, void *gctx, struct ut_s *ut_info, struct ut_global_config_s const *ut_gconf,        \
...
#define unittest( ... )                                                                            \
    ...
    static struct ut_s const ut_build_name(                                                        \
        ut_info_, UNITTEST_UNIQUE_ID,                                                              \
        __LINE__ ) = {/* file, unique_id, line, exec, fn, name, depends_on */                      \
                      __FILE__,                                                                    \
                      UNITTEST_UNIQUE_ID,                                                          \
                      __LINE__,                                                                    \
                      1,                                                                           \
                      ut_build_name( ut_body_, UNITTEST_UNIQUE_ID, __LINE__ ),                     \
                      0,                                                                           \
                      0,                                                                           \
                      0,                                                                           \
                      __VA_ARGS__};                                                                \
...
#define unittest_config( ... )                                                                     \
    static struct ut_group_config_s const ut_build_name( ut_config_, UNITTEST_UNIQUE_ID,           \
                                                         __LINE__ ) = {                            \
        .file = __FILE__, .line = __LINE__, .unique_id = UNITTEST_UNIQUE_ID, __VA_ARGS__};         \
...
static void ut_print_assertion_failed( struct ut_s const *info,
                                       struct ut_global_config_s const *gconf,
                                       struct ut_group_config_s const *config, int64_t line,
                                       char const *func, char const *expr, char const *fmt, ... )
{
    ...
    fprintf( gconf->fp,
             ut_color( UT_YELLOW, "assertion failed" ) ": [%s] %s:" ut_color(
                 UT_BLUE, "%" PRId64 "" ) " (%s) `" ut_color( UT_MAGENTA, "%s" ) "'",
             ut_null_replace( config->name, "no name" ),
             ut_null_replace( info->file, "(unknown filename)" ), line,
             ut_null_replace( info->name, "no name" ), expr );
    if ( strlen( fmt ) != 0 )
    {
        fprintf( gconf->fp, ", " );
...
static void ut_print_assertion_failed_json( struct ut_s const *info,
                                            struct ut_global_config_s const *gconf,
                                            struct ut_group_config_s const *config, int64_t line,
                                            char const *func, char const *expr, char const *fmt,
                                            ... )
...
/* summary printers */
static void ut_print_results( struct ut_global_config_s const *gconf,
                              struct ut_group_config_s const *config,
                              struct ut_result_s const *result, int64_t file_cnt )
{
    int64_t cnt = 0;
    int64_t succ = 0;
    int64_t fail = 0;
    ...
    for ( int64_t i = 0, j = 0; i < file_cnt; i++ )
    {
        ...
        fprintf( gconf->fp,
                 "%sGroup %s: %" PRId64 " succeeded, %" PRId64 " failed in total %" PRId64
                 " assertions in %" PRId64 " tests.%s\n",
                 ( result[ j ].fail == 0 ) ? UT_GREEN : UT_RED,
                 ut_null_replace( config[ i ].name, "(no name)" ), result[ j ].succ,
                 result[ j ].fail, result[ j ].succ + result[ j ].fail, result[ j ].cnt,
                 UT_DEFAULT_COLOR );
    ...
    fprintf( gconf->fp,
             "%sSummary: %" PRId64 " succeeded, %" PRId64 " failed in total %" PRId64
             " assertions in %" PRId64 " tests.%s\n",
             ( fail == 0 ) ? UT_GREEN : UT_RED, succ, fail, succ + fail, cnt, UT_DEFAULT_COLOR );
    return;
}

static void ut_print_results_json( struct ut_global_config_s const *gconf,
                                   struct ut_group_config_s const *config,
                                   struct ut_result_s const *result, int64_t file_cnt )
{
    int64_t cnt = 0;
...
static struct ut_printer_s ut_default_printer = {.failed = ut_print_assertion_failed,
                                                 .result = ut_print_results};

static struct ut_printer_s ut_json_printer = {.failed = ut_print_assertion_failed_json,
...
#define ut_dump( ptr, len )                                                                        \
    ( {                                                                                            \
        uint64_t _size =                                                                           \
            ( ( ( len ) + 15 ) / 16 + 1 ) * ( strlen( "0x0123456789abcdef:" ) +                    \
                                              16 * strlen( " 00a" ) + strlen( "  \n+ margin" ) ) + \
            strlen( #ptr ) + strlen( "\n`' len: 100000000" );                                      \
...
#define ut_assert( expr, ... )                                                                     \
    {                                                                                              \
        ...
        {                                                                                          \
            ...
            ut_gconf->printer.failed( ut_info, ut_gconf, ut_config, __LINE__, __func__, #expr,     \
                                      "" __VA_ARGS__ );                                            \
        }                                                                                          \
    }
#ifndef assert
...
static inline int ut_strcmp( char const *a, char const *b )
{
    /* if both are NULL */
    if ( a == NULL && b == NULL )
    {
...
static inline char *ut_build_nm_cmd( char const *filename )
{
...
static inline char *ut_dump_file( FILE *fp )
{
    int c;
    utkvec_t( char ) buf;
...
static inline char *ut_dump_nm_output( char const *filename )
{
    char *cmd = NULL;
    FILE *fp = NULL;
    char *res = NULL;
...
static inline struct ut_nm_result_s *ut_parse_nm_output( char const *str )
{
    char const *p = str;
...
static inline struct ut_nm_result_s *ut_nm( char const *filename )
{
    char const *str = NULL;
...
static inline int ut_startswith( char const *str, char const *prefix )
{
...
static inline struct ut_s *ut_get_unittest( struct ut_nm_result_s const *res )
{
    /* extract offset */
...
static inline struct ut_group_config_s *ut_get_ut_config( struct ut_nm_result_s const *res )
{
    /* extract offset */
    uintptr_t offset = ( uintptr_t )-1LL;
    struct ut_nm_result_s const *r = res;
...
#define ut_get_config_call_func( _ptr, _offset )                                                   \
    ( ( struct ut_group_config_s( * )( void ) )( ( uintptr_t )( _ptr ) +                           \
                                                 ( uintptr_t )( _offset ) ) )
...
static inline void ut_dump_test( struct ut_s const *test )
{
    struct ut_s const *t = test;
    while ( t->file != NULL )
    {
        printf( "%s, %" PRIu64 ", %" PRId64 ", %s, %s, %p, %p\n", t->file, t->line, t->unique_id,
                t->name, t->depends_on[ 0 ], ( void * )t->init, ( void * )t->clean );
        t++;
    }
    return;
}

static inline void ut_dump_config( struct ut_group_config_s const *config )
{
    struct ut_group_config_s const *c = config;
    while ( c->file != NULL )
    {
        printf( "%s, %" PRId64 ", %s, %s, %p, %p\n", c->file, c->unique_id, c->name,
                c->depends_on[ 0 ], ( void * )c->init, ( void * )c->clean );
        c++;
    }
    return;
}

static int ut_compare( void const *_a, void const *_b )
{
...
#define sat( a )                                                                                   \
    ( ( ( a ) > INT32_MAX ) ? INT32_MAX : ( ( ( a ) < INT32_MIN ) ? INT32_MIN : ( a ) ) )
...
static inline int ut_match( void const *_a, void const *_b )
{
...
static inline uint64_t ut_get_total_test_count( struct ut_s const *test )
{
    uint64_t cnt = 0;
    struct ut_s const *t = test;
    while ( t->file != NULL )
...
static inline uint64_t ut_get_total_config_count( struct ut_group_config_s const *config )
{
    uint64_t cnt = 0;
    struct ut_group_config_s const *c = config;
    while ( c->file != NULL )
...
static inline uint64_t ut_get_total_file_count( struct ut_s const *sorted_test )
{
    uint64_t cnt = 1;
...
static inline void ut_sort( struct ut_s *test, struct ut_group_config_s *config )
{
    // ut_dump_test(test);
    qsort( test, ut_get_total_test_count( test ), sizeof( struct ut_s ), ut_compare );
    qsort( config, ut_get_total_config_count( config ), sizeof( struct ut_group_config_s ),
           ut_compare );
    // ut_dump_test(test);
...
static inline uint64_t *ut_build_file_index( struct ut_s const *sorted_test )
{
    uint64_t cnt = 1;
...
static inline struct ut_group_config_s *ut_compensate_config(
    struct ut_s *sorted_test, struct ut_group_config_s *sorted_config )
{
...
static inline int ut_toposort_by_tag( struct ut_s *sorted_test, uint64_t test_cnt )
{
    /* init dag */
    ...
    for ( uint64_t i = 0; i < test_cnt; i++ )
    {
        ...
        if ( node_id == ( uint64_t )-1LL )
        {
            fprintf( stderr,
                     ut_color( UT_RED,
                               "ERROR" ) ": detected circular dependency in the tests in "
                                         "`" ut_color( UT_MAGENTA, "%s" ) "'.\n",
                     sorted_test[ 0 ].file );
            return ( -1 );
        }
    ...
    if ( utkv_size( res ) != test_cnt )
    {
        fprintf(
            stderr,
            ut_color( UT_RED, "ERROR" ) ": detected circular dependency in the tests in `" ut_color(
                UT_MAGENTA, "%s" ) "'.\n",
            sorted_test[ 0 ].file );
        return ( -1 );
    }
...
static inline int ut_toposort_by_group( struct ut_s *sorted_test, uint64_t test_cnt,
                                        struct ut_group_config_s *sorted_config, uint64_t *file_idx,
                                        uint64_t file_cnt )
{
    /* init dag */
    utkvec_t( utkvec_t( uint64_t ) ) dag;
    ...
    for ( uint64_t i = 0; i < file_cnt; i++ )
    {
        ...
        if ( file_id == ( uint64_t )-1LL )
        {
            fprintf(
                stderr,
                ut_color( UT_RED, "ERROR" ) ": detected circular dependency between groups.\n" );
            return ( -1 );
        }
 ...
 */
static inline char *ut_build_short_option_string( struct option const *opts )
{
    char *str = NULL, *ps;
    uint64_t len = 0;
 ...
 */
static inline int ut_modify_test_config_mark( char const *arg, void *_test, int64_t cnt )
{
 ...
 */
static inline int ut_modify_test_config_all( void *_test, int64_t cnt )
{
    struct ut_s *test = ( struct ut_s * )_test;
    for ( int64_t i = 0; i < cnt; i++ )
    {
 ...
 * @fn ut_print_help
 */
static inline void ut_print_help( void )
{
 ...
 */
static inline int ut_modify_test_config( int argc, char *const argv[],
                                         struct ut_global_config_s *params,
                                         struct ut_s *sorted_test, int64_t test_cnt,
                                         struct ut_group_config_s *sorted_config, int64_t file_cnt )
{
    static struct option const opts_long[] = {
        {"group", required_argument, NULL, 'g'}, {"test", required_argument, NULL, 't'},
        {"seed", required_argument, NULL, 's'},  {"stdout", no_argument, NULL, 'o'},
        {"json", no_argument, NULL, 'j'},        {"threads", required_argument, NULL, 'n'},
    ...
    while ( ( c = getopt_long( argc, argv, opts_short, opts_long, &idx ) ) != -1 )
    {
        switch ( c )
        {
            ...
            case 't':
                ...
                break;
            case 's':
                srand( ( unsigned int )atol( optarg ) );
                break;
            case 'j':
                params->printer = ut_json_printer;
                break;
            case 'o':
                params->fp = stdout;
                break;
 ...
 */
static inline void ut_propagate_config( struct ut_s *test, int64_t test_cnt,
                                        struct ut_group_config_s *compd_config,
                                        uint64_t *sorted_file_idx, int64_t file_cnt )
{
    /* set index */
    for ( uint64_t i = 0; i < file_cnt; i++ )
    {
 ...
 * @fn ut_run_test
 */
static void ut_run_test( struct ut_s *test, struct ut_global_config_s const *gconf,
 ...
 * @fn ut_main_impl
 */
static int ut_main_impl( int argc, char *argv[] )
{
    /* dump symbol table */
    ...
    if ( ut_toposort_by_group( test, test_cnt, compd_config, file_idx, file_cnt ) != 0 )
    {
        fprintf(
            ...
            ut_color(
                UT_RED,
                "ERROR" ) ": failed to order tests. check if the depends_on options are sane.\n" );
        return ( 1 );
    }
    ...
    /* init default params */
    struct ut_global_config_s gconf = {.fp = stderr, .printer = ut_default_printer};
...
static int unittest_main( int argc, char *argv[] )
{
