 ...
 * @param[in] bw : bandwidth (the alignment path of the input sequence and the result does not go
 * out of the band)
 *
...
int main( int argc, char *argv[] )
{
    ...
    /** init context */
    gaba_t *ctx = gaba_init( GABA_PARAMS(.xdrop = 100, GABA_SCORE_SIMPLE( 2, 3, 5, 1 ) ) );
    ...
    printf( "%" PRId64 "\t%" PRId64 "\t%" PRId64 "\t%" PRId64 "\t%" PRId64 "\n", bench_get( fill ),
            bench_get( trace ), bench_get( parse ),
            bench_get( fill ) + bench_get( trace ) + bench_get( parse ), score );
